...
│   │   ├── ItemDetailView.swift        # 详情：价值/历史/预测/折旧
│   │   ├── ItemEditView.swift          # 完整新增/编辑
│   │   ├── QuickAddSheet.swift         # 模板快速新增
│   │   ├── RestockSheet.swift          # 一键补货
│   │   ├── PurchaseEditSheet.swift     # 购买记录编辑
│   │   ├── InsightsView.swift          # 洞察图表
│   │   └── ProfileView.swift           # 设置/模板/导出/权限/清理
│   ├── Components/
│   │   ├── ItemCard.swift              # 业务卡片
...
项目在架构上为以下方向预留了空间（当前未实现，作为课程演示的"下一步"）：

- **iCloud 同步**：SwiftData + CloudKit 兼容 schema
- **商家管理**：Merchant 模型已就绪，UI 可补全
- **节省统计**：以历史中位单价为基准的估算
- **成就/等级**：连续无断货天数等轻量 gamification
- **拍照保存**：PhotosPicker 已纳入权限说明，UI 可接入
