...
> 一款管理"物品与服务的生命周期、使用价值和下一次购买决策"的应用。
...
- **小票 OCR 记账**：Vision 框架识别小票金额，一键填入补货/购买记录（设备端，不上传）
- **跨物品单价对比**：同类消耗品单价横向比较，标出最划算的一档
- **预测 vs 实际双线图**：回测历史预测准确率，展示 MAE / MAPE
- **提醒中心**：按紧迫度统一查看所有待提醒物品，支持稍后/已处理快捷操作
...
已实现 ✅ 的功能：商家管理、节省统计、成就/等级、物品图片（PhotosPicker）、首次引导、小票 OCR 记账、跨物品单价对比、预测 vs 实际双线图、提醒中心。

项目在架构上仍为以下方向预留了空间（作为课程演示的"下一步"）：

- **iCloud 同步**：SwiftData + CloudKit 兼容 schema（需付费 Apple Developer 账号配置 CloudKit 容器）
- **真实商品照片**：当前演示用 SF Symbol 占位缩略图，可替换为 Assets 中的真实照片
- **复购间隔趋势图**：在详情页补充展示历史间隔的折线变化
